...
import jitkrittum.util as util
...
class LinearMMDTest(TwoSampleTest):
    ...
    def two_moments(X, Y, kernel):
        ...
        if isinstance(X, np.ndarray):
            ...
            Xodd = X[::2]
            Xeven = X[1::2]
            assert len(Xodd) == len(Xeven)
            Yodd = Y[::2]
            Yeven = Y[1::2]
            assert len(Yodd) == len(Yeven)
        # linear mmd. O(n)
...
class MeanEmbeddingTest(TwoSampleTest):
    ...
    def init_locs_randn(tst_data, n_test_locs, seed=1):
        ...
        if d <= 5000: # empirically, declare all dimensions under 5k to be suitable for conventional init
            cov_xy = np.cov(xy.T)
            [Dxy, Vxy] = np.linalg.eig(cov_xy + 1e-3*np.eye(d))
            Dxy = np.real(Dxy)
            Vxy = np.real(Vxy)
            Dxy[Dxy<=0] = 1e-3
            # Below used to be 0.9.
            # Changed as shrinkage can lead to weird behaviour downstream.
            ...
            var_xy = np.var(xy,axis=0) # Don't use the covariance structure to save memory!
            T0 = mean_xy + np.sqrt(var_xy)*np.random.randn(n_test_locs,d)
        # reset the seed back to the original
        np.random.set_state(rand_state)
        return T0
