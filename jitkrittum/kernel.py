...
# class GAK(Kernel):
#     """Global alignment kernel for time series data"""
#     def __init__(self,sigma=1.,n_jobs=cpu_count()):
#         self.sigma = sigma
#         self.n_jobs = n_jobs
#     def eval(self, X1, X2):
#         return cdist_gak(X1,X2,sigma=self.sigma,n_jobs=self.n_jobs)
#     def pair_eval(self, X, Y):
#         vals = Parallel(n_jobs=self.n_jobs)(delayed(gak)(X[i],Y[i],sigma=self.sigma) for i in range(len(X)))
#         return np.hstack(vals)
#     def __str__(self):
#         return f'GAK(sigma={self.sigma})'
