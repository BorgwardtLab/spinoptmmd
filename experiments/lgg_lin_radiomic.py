...
print('Experiment: LGG radiomic, linear')
...
for label in ['H0','ATRX']: #,'TP53']:
    ...
    np.savetxt(os.path.join(OUT_DIR,f'mean_contrast_names_lgg_rad_{label}_lin_lam_{lam}.txt'),mean_contrast_names)
    ...
    out_df_baseline.to_csv(os.path.join(OUT_DIR,f'lgg_rad_{label}_lin_baseline_lam_{lam}.csv'),index=False)
    out_df.to_csv(os.path.join(OUT_DIR,f'lgg_rad_{label}_lin_lam_{lam}.csv'),index=False)