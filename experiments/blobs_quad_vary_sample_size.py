...
print('Experiment: Blobs vary sample size, quadratic')
...
for i,sample_size in enumerate(sample_size_list):
    ...
    for j in tqdm(range(n_trials)):
        ...
        tst_data = data.SSBlobs().sample(sample_size,seed)
...
out_df_baseline.to_csv(os.path.join(OUT_DIR,'blobs_quad_vary_sample_size_baseline.csv'),index=False)
out_df.to_csv(os.path.join(OUT_DIR,'blobs_quad_vary_sample_size.csv'),index=False)