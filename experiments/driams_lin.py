...
from src.utils import load_driams
...
file_path = os.path.join('data','DRIAMS')

print('Experiment: DRIAMS, linear')

for label in ['H0','test']:
    for s,d in zip(['Escherichia coli', 'Klebsiella pneumoniae', 'Staphylococcus aureus'],['Ceftriaxone', 'Ceftriaxone','Oxacillin']):
        ...
        X,Y = load_driams(file_path,species=s,drug=d,label=label)
        ...
        for j in tqdm(range(n_trials),disable=False):
            ...
            op = {
                ...
                'lam': lam*6000, # LASSO regularization strength
            ...
            contrast_names = np.zeros(6000)
            if len(feature_ids)>0:
                contrast_names[feature_ids] = 1
                median_dist_pruned = tst.LinearMMDTest.median_distance(tr.X*contrast_names, tr.Y*contrast_names)
                k_list_pruned = [kernel.KGauss(sigma2=s) for s in median_dist_pruned*2**np.linspace(-4,4,30)]
                k_idx_pruned,_ = tst.LinearMMDTest.grid_search_kernel(tr.X*contrast_names,tr.Y*contrast_names,k_list_pruned,alpha)
                k_ours_pruned = k_list_pruned[k_idx_pruned]
                pruned_test_baseline = tst.LinearMMDTest(kernel=k_ours_pruned,alpha=alpha).perform_test(te,w=contrast_names)
                masked_weights = np.zeros(6000)
        ...
        np.savetxt(os.path.join(OUT_DIR,f'mean_contrast_names_driams_{s}_{d}_{label}_lin_lam_{lam}.txt'),mean_contrast_names)

        out_df_baseline.loc[0,'alpha'] = alpha
        out_df_baseline.loc[0,'d'] = 6000
        ...
        out_df_baseline.to_csv(os.path.join(OUT_DIR,f'driams_{s}_{d}_{label}_lin_baseline_lam_{lam}.csv'),index=False)
        out_df.to_csv(os.path.join(OUT_DIR,f'driams_{s}_{d}_{label}_lin_lam_{lam}.csv'),index=False)