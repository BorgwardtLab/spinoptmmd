...
print('Experiment: NIPS, Jitkrittum')
...
for fname in nips_files:
    ...
    for j in tqdm(range(n_trials),disable=False):
        ...
        contrast_words_list.append((v > np.quantile(v,0.9)).astype(int))
    ...
    mean_contrast_words = np.nanmean(np.array(contrast_words_list),axis=0)
    np.savetxt(os.path.join(OUT_DIR,f'mean_contrast_words_{categories[0]}_{categories[1]}.txt'),mean_contrast_words)
    ...
    out_df_baseline.to_csv(os.path.join(OUT_DIR,f'nips_{categories[0]}_{categories[1]}.csv'),index=False)
