...
print('Experiment: ADNI radiomic, Jitkrittum')
...
for label in ['H0','test']:
    ...
    np.savetxt(os.path.join(OUT_DIR,f'mean_contrast_names_adni_rad_{label}.txt'),mean_contrast_names)
    ...
    out_df_baseline.to_csv(os.path.join(OUT_DIR,f'adni_rad_{label}.csv'),index=False)
