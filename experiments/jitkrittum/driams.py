...
print('Experiment: DRIAMS, Jitkrittum')

for label in ['H0','test']:
    for s,d in zip(['Escherichia coli', 'Staphylococcus aureus'],['Ceftriaxone','Oxacillin']):
        cols = ['alpha','pvalue','pvalue_std','test_stat','h0_rejected','lambda','d','sample_size','runtime','runtime_std']
        ...
        np.savetxt(os.path.join(OUT_DIR,f'mean_contrast_names_driams_{s}_{d}_{label}.txt'),mean_contrast_names)

        out_df_baseline.loc[0,'alpha'] = alpha
        out_df_baseline.loc[0,'d'] = tst_data.X.shape[1]
        ...
        out_df_baseline.to_csv(os.path.join(OUT_DIR,f'driams_{s}_{d}_{label}.csv'),index=False)
