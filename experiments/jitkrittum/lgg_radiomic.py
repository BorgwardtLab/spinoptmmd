...
print('Experiment: LGG radiomic, Jitkrittum')
...
for label in ['H0','ATRX']:
    ...
    np.savetxt(os.path.join(OUT_DIR,f'mean_contrast_names_lgg_rad_{label}.txt'),mean_contrast_names)
    ...
    out_df_baseline.to_csv(os.path.join(OUT_DIR,f'lgg_rad_{label}.csv'),index=False)
