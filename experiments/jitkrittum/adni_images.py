...
print('Experiment: ADNI images, Jitkrittum')
...
for label in ['H0','test']:
    ...
    for j in tqdm(range(n_trials),disable=False):
        ...
        contrast_pixels_list.append((v > np.quantile(v,0.9)).astype(int))
    ...
    mean_contrast_pixels = np.nanmean(np.array(contrast_pixels_list),axis=0)
    np.savetxt(os.path.join(OUT_DIR,f'mean_contrast_pixels_adni_img_{label}.txt'),mean_contrast_pixels)
    ...
    out_df_baseline.to_csv(os.path.join(OUT_DIR,f'adni_img_{label}.csv'),index=False)
