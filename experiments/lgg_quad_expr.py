...
print('Experiment: LGG expression, quadratic')
...
for label in ['H0','ATRX']: #,'TP53']:
    ...
    np.savetxt(os.path.join(OUT_DIR,f'mean_contrast_names_lgg_expr_{label}_quad_lam_{lam}.txt'),mean_contrast_names)
    ...
    out_df_baseline.to_csv(os.path.join(OUT_DIR,f'lgg_expr_{label}_quad_baseline_lam_{lam}.csv'),index=False)
    out_df.to_csv(os.path.join(OUT_DIR,f'lgg_expr_{label}_quad_lam_{lam}.csv'),index=False)