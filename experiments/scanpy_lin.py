...
print('Experiment: Scanpy, linear')
...
for label in ['H0']+list(range(8)):
    ...
    np.savetxt(os.path.join(OUT_DIR,f'mean_contrast_names_scanpy_{label}_lin_lam_{lam}.txt'),mean_contrast_names)
    ...
    out_df_baseline.to_csv(os.path.join(OUT_DIR,f'scanpy_{label}_lin_baseline_lam_{lam}.csv'),index=False)
    out_df.to_csv(os.path.join(OUT_DIR,f'scanpy_{label}_lin_lam_{lam}.csv'),index=False)