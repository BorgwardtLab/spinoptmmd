...
from src.utils import load_adni_radiomic
...
print('Experiment: ADNI radiomic, linear')
...
for label in ['H0','test']:
    ...
    X,Y,names = load_adni_radiomic(file_path,label=label)
    ...
    np.savetxt(os.path.join(OUT_DIR,f'mean_contrast_names_adni_rad_{label}_lin_lam_{lam}.txt'),mean_contrast_names)
    ...
    out_df_baseline.to_csv(os.path.join(OUT_DIR,f'adni_rad_{label}_lin_baseline_lam_{lam}.csv'),index=False)
    out_df.to_csv(os.path.join(OUT_DIR,f'adni_rad_{label}_lin_lam_{lam}.csv'),index=False)