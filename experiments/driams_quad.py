...
print('Experiment: DRIAMS, quadratic')
...
for label in ['H0','test']:
    for s,d in zip(['Escherichia coli', 'Klebsiella pneumoniae', 'Staphylococcus aureus'],['Ceftriaxone', 'Ceftriaxone','Oxacillin']):
        ...
        np.savetxt(os.path.join(OUT_DIR,f'mean_contrast_names_driams_{s}_{d}_{label}_quad_lam_{lam}.txt'),mean_contrast_names)
        ...
        out_df_baseline.to_csv(os.path.join(OUT_DIR,f'driams_{s}_{d}_{label}_quad_baseline_lam_{lam}.csv'),index=False)
        out_df.to_csv(os.path.join(OUT_DIR,f'driams_{s}_{d}_{label}_quad_lam_{lam}.csv'),index=False)