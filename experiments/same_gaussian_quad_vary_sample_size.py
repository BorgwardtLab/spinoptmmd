...
out_df = pd.DataFrame(np.zeros((len(sample_size_list),len(cols))),columns=cols)
out_df_baseline = pd.DataFrame(np.zeros((len(sample_size_list),len(cols_baseline))),columns=cols_baseline)

print('Experiment: Same gauss vary sample size, quadratic')
...
for i,sample_size in enumerate(sample_size_list):
    ...
    out_df.loc[i,['pvalue','test_stat','h0_rejected']] = results_df.mean(axis=0)
...
out_df_baseline.to_csv(os.path.join(OUT_DIR,'same_gaussian_quad_vary_sample_size_baseline.csv'),index=False)
out_df.to_csv(os.path.join(OUT_DIR,'same_gaussian_quad_vary_sample_size.csv'),index=False)