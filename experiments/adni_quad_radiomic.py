...
print('Experiment: ADNI radiomic, quadratic')
...
for label in ['H0','test']:
    ...
    np.savetxt(os.path.join(OUT_DIR,f'mean_contrast_names_adni_rad_{label}_quad_lam_{lam}.txt'),mean_contrast_names)
    ...
    out_df_baseline.to_csv(os.path.join(OUT_DIR,f'adni_rad_{label}_quad_baseline_lam_{lam}.csv'),index=False)
    out_df.to_csv(os.path.join(OUT_DIR,f'adni_rad_{label}_quad_lam_{lam}.csv'),index=False)