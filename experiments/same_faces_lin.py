...
print('Experiment: Same faces, linear')
...
for j in tqdm(range(n_trials),disable=False):
    ...
    if len(feature_ids)>0:
        ...
        median_dist_pruned = tst.LinearMMDTest.median_distance(tr.X*contrast_image, tr.Y*contrast_image)
        k_list_pruned = [kernel.KGauss(sigma2=s) for s in median_dist_pruned*2**np.linspace(-4,4,30)]
        k_idx_pruned,_ = tst.LinearMMDTest.grid_search_kernel(tr.X*contrast_image,tr.Y*contrast_image,k_list_pruned,alpha)
        k_ours_pruned = k_list_pruned[k_idx_pruned]
        pruned_test_baseline = tst.LinearMMDTest(kernel=k_ours_pruned,alpha=alpha).perform_test(te,w=contrast_image)
...
np.savetxt(os.path.join(OUT_DIR,f'mean_contrast_image_same_faces_lin_lam_{lam}.txt'),mean_contrast_image)
...
# To test
# plt.figure()
# plt.imshow(mean_contrast_image.reshape(y_orig,x_orig),cmap='binary')
# plt.show()
out_df_baseline.to_csv(os.path.join(OUT_DIR,f'same_faces_lin_baseline_lam_{lam}.csv'),index=False)
out_df.to_csv(os.path.join(OUT_DIR,f'same_faces_lin_lam_{lam}.csv'),index=False)