...
from src.utils import load_lgg_expression
...
print('Experiment: LGG expression, linear')
...
for label in ['H0','ATRX']: #,'TP53']:
    ...
    X,Y,names = load_lgg_expression(file_path,label=label)
    ...
    for j in tqdm(range(n_trials),disable=False):
        ...
        k_list = [kernel.KGauss(sigma2=s) for s in median_dist*np.arange(0.1,2.1,0.1)]
    ...
    np.savetxt(os.path.join(OUT_DIR,f'mean_contrast_names_lgg_expr_{label}_lin_lam_{lam}.txt'),mean_contrast_names)
    ...
    out_df_baseline.to_csv(os.path.join(OUT_DIR,f'lgg_expr_{label}_lin_baseline_lam_{lam}.csv'),index=False)
    out_df.to_csv(os.path.join(OUT_DIR,f'lgg_expr_{label}_lin_lam_{lam}.csv'),index=False)