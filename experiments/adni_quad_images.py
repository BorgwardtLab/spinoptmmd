...
print('Experiment: ADNI images, quadratic')
...
for label in ['H0','test']:
    ...
    for j in tqdm(range(n_trials),disable=False):
        ...
        if len(feature_ids)>0:
            ...
            median_dist_pruned = tst.QuadMMDTest.median_distance(tr.X*contrast_pixels, tr.Y*contrast_pixels)
            k_list_pruned = [kernel.KGauss(sigma2=s) for s in median_dist_pruned*2**np.linspace(-4,4,30)]
            k_idx_pruned,_ = tst.QuadMMDTest.grid_search_kernel(tr.X*contrast_pixels,tr.Y*contrast_pixels,k_list_pruned,alpha,func_var=tst.QuadMMDTest.compute_var_biased)
            k_ours_pruned = k_list_pruned[k_idx_pruned]
            pruned_test_baseline = tst.QuadMMDTest(kernel=k_ours_pruned,alpha=alpha).perform_test(te,w=contrast_pixels)
    ...
    np.savetxt(os.path.join(OUT_DIR,f'mean_contrast_pixels_adni_img_{label}_quad_lam_{lam}.txt'),mean_contrast_pixels)
    ...
    out_df_baseline.to_csv(os.path.join(OUT_DIR,f'adni_img_{label}_quad_baseline_lam_{lam}.csv'),index=False)
    out_df.to_csv(os.path.join(OUT_DIR,f'adni_img_{label}_quad_lam_{lam}.csv'),index=False)